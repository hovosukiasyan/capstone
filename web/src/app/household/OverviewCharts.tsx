...
import SeasonalBar from '@/components/charts/SeasonalBar';
import { apiFetcher, getErrorMessage } from '@/lib/fetcher';
import type { DistributionResponse } from '@/lib/types';

interface Props {
  initialByMonth: { month: number; mean_income: number; count: number }[];
}

export default function HouseholdOverviewCharts({ initialByMonth }: Props) {
  ...
  return (
    <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
      ...
      {/* Seasonal bar */}
      <div className="rounded-xl border border-slate-200 bg-white p-6">
        <h3 className="text-sm font-semibold text-slate-700 mb-1">Seasonal Income Pattern</h3>
        <p className="text-xs text-slate-400 mb-4">Mean household income by survey month</p>
        <SeasonalBar data={initialByMonth} />
        <p className="mt-3 text-xs text-slate-400">
          Nov/Dec highest (harvest + remittances). Jan–Mar lowest (post-winter dip).
        </p>
      </div>
    </div>
  );
